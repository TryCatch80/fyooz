...
/**
 *
 * This is the entrance point to jobgrokapp
 *
...
if ($isMenuItem) {
    ...
    if ($result = $db->loadObject()) {
        ...
        foreach ($p as $pp) {
                if ($pp == 'index.php?option=com_jobgrokapp') $isMenuItem = true;
        }
    } else {
        $isMenuItem = false;
    }
...
if ($c = JRequest :: getCmd('controller', $default_controller))
{
    ...
    $c = 'JobgrokappController' . $default_controller;
    $controller = new $c ();
...
{
    JError::raiseError('500', JTEXT::_('COM_JOBGROKAPP_JOBGROK_NO_CONTROLLER_FOUND'));
}
