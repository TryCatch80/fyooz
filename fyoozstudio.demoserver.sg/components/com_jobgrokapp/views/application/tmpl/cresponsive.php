    	...
    	<div class="row">
    		<div class="position-app-center">                	
    			<div class="enquiries-form-box">                    	
                    ...
                    <?php endif; ?>
                	<div class="form-fild">
                    	<textarea name="interest" id="interest" cols="" rows="" placeholder="About Me"></textarea>
					</div>
                    <?php 
						$db = JFactory::getDbo();
                    ...
                    <fieldset id="jg_references">
                        ...
                        <?php endif; ?>
                                        ...
                                        <div class="span12">
                                            <?php /*?><?php echo $this->params->get('field_upload_file_desc',JTEXT::_('COM_JOBGROKAPP_VIEWS_APPLICATION_TMPL_CRESPONSIVE_UPLOAD_FILE_DESC')); ?>&nbsp;<em>(<?php
                                              ...
                                              <!--   <input type="hidden" name="file_name" id="file_name" value="<?php if (isset($this->application->file_name)) echo $this->application->file_name; ?>" /> -->
                                               <input id="file_name_d" name="file_name_d" value="" class="disableInputField" style="width:100%;" placeholder="Attach resume*" disabled="disabled"/>
                                          <!--      <input id="file_name_d" name="file_name_d" value="<?php if (isset($this->application->file_name)) echo $this->application->file_name; ?>" class="disableInputField" style="width:100%;" placeholder="Attach resume*" disabled="disabled"/> -->
                                                <label class="fileUpload"><input type="file" onchange="document.getElementById('file_name').value=this.value" name="file_upload" id="file_upload" title="Pick a file to upload" value="" class="upload"/></label>
													...
													jQuery(function() {
														...
														//	allowedExtensions: ['doc', 'docx', 'pdf'],
															...
															error: function(e) {
																...
																{
																jQuery('#file_name').parents('.form-fild').find('.compulsory-msg-box').remove();
																jQuery('#file_name').parents('.form-fild').append('<div class="compulsory-msg-box"><div class="compulsory-msg block-err">Please upload valid resume file.</br> Allowed 5 mb File Size.</div></div>');
																jQuery('#file_name').val('');	
																}
															}
														});
                            ...
                            <?php endif; ?>
                                ...
                                <div class="form-fild">
								<div class="span12 jg_personal_information_first_name">
								<div class="control-group">
								<input size="40" class="span12 enquiries-input" type="text" name="captcha_entered" id="captcha_entered" maxlength="8" value="" placeholder="Enter Security Code Here" />
                                </div>
                                <span class="capch_img2"><img name="captcha_img" id="captcha_img" alt="captchmy" src="<?php echo 'index.php?option=com_jobgrokapp&amp;view=application&amp;format=captcha&amp;Itemid='.JRequest::getVar('Itemid'); ?>" /></span>
								</div>
								</div>
                            ...
                            <table width="100%" class="admintable">
								...
								$success = $session->get('enq_success');
							 	$enq_error = $session->get('enq_error');
								$enq_msg = $session->get('enq_msg');
								if($enq_error==1)
								{
										?>
                                <div id="message">
                                    <div class="compulsory-msg-box">
                                        <div class="compulsory-msg block-err"><?php echo $enq_msg; ?></div>
                                    </div>
                                </div>
                                <?php }?>
                                <?php /*?><tr>
...
function checkValid()
{
	...
	var captcha_code = jQuery('#captcha_entered').val();
	
	if(first_name != '' && home_phone != '' && email_address != '' && emailInvalid(email_address) && file_name != '' && captcha_code!='')
	{
	...
	else
	{
		...
		if(captcha_code=="")
		{
			...
			jQuery('#captcha_entered').parents('.inputBtnSection').addClass('error');
			jQuery('#captcha_entered').parents('.form-fild').find('.compulsory-msg-box').remove();
			jQuery('#captcha_entered').parents('.form-fild').append('<div class="compulsory-msg-box"><div class="compulsory-msg block-err">Please enter captcha code.</div></div>');
		}
		else
		{
			jQuery('#captcha_entered').removeClass('error');
			jQuery('#captcha_entered').parents('.form-fild').find('.compulsory-msg-box').remove();
		}
        ...
        return this.each(function() {
            ...
            jQuery(this).on('change', function(e) {
				...
				var value = jQuery(this).val(),					
					...
					/* added to check proper pdf with mime */
					try{
						...
						fileReader.onloadend = function(e) {
						  ...
						  console.log(header);
						  if(parseInt(header)!=25504446){//25504446 2D
								options.error(1);
								jQuery(this).focus();							  
						  }
				   ...
				   else if (jQuery.inArray(extension, options.allowedExtensions) == -1) {
